...
const SushiBarMarket = artifacts.require('SushiBarMarket')
const SushiBarMock = artifacts.require('SushiBarMock')
...
contract('SushiBar', accounts => {
  ...
  const timePass = async (timeInYears) => {
    ...
    await stablecoin.mint(sushibar.address, num2str(BigNumber(await stablecoin.balanceOf(sushibar.address)).times(INIT_INTEREST_RATE).times(timeInYears)))
  }

  beforeEach(async function () {
    // Initialize mock stablecoin (sushi) and sushibar (xsushi)
    stablecoin = await ERC20Mock.new()
    sushibar = await SushiBarMock.new(stablecoin.address)
    ...
    market = await SushiBarMarket.new(stablecoin.address, sushibar.address)
